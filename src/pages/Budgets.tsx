...
export function Budgets() {
  ...
  const unbudgetedCategories = categories.filter(c => !budgets.some(b => b.categoryId === c.id) || b.id === editingId);

  return (
    <div className="p-4 space-y-4 relative min-h-full pb-20">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-semibold tracking-tight">Budgets</h1>
        <Button size="sm" onClick={() => { resetForm(); setIsModalOpen(true); }} className="bg-accent hover:bg-accent-hover text-white">
      ...
      <div className="space-y-4">
        {budgets.map(budget => {
          ...
          return (
            <Card key={budget.id} className="cursor-pointer hover:border-accent transition-colors" onClick={() => handleOpenEdit(budget)}>
                ...
                <div className="flex justify-between items-center mb-2">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-primary-muted">{percent >= 100 ? 'Over budget' : `${(100 - percent).toFixed(0)}% remaining`}</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="font-semibold">₦{spent.toLocaleString()}</div>
                    <div className="text-xs text-primary-muted">of ₦{budget.limitAmount.toLocaleString()}</div>
                  </div>
                </div>
          ...
          <>
            ...
            >
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-semibold">{editingId ? 'Edit Budget' : 'Set Budget'}</h2>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  >
                    ...
                    {categories.map(c => (
                      <option key={c.id} value={c.id} disabled={!unbudgetedCategories.some(uc => uc.id === c.id) && c.id !== categoryId}>
                        {c.icon} {c.name}
                      </option>
                    ))}
                  </select>
                ...
                <div>
                  <label className="text-sm font-medium mb-1 block">Monthly Limit (₦)</label>
                  <Input type="number" placeholder="50000" value={amount} onChange={(e) => setAmount(e.target.value)} />
                </div>

                <div className="pt-4 flex space-x-3">
                  {editingId && (
                    <Button variant="danger" size="icon" onClick={() => { deleteBudget(editingId); setIsModalOpen(false); }}>
                      <Trash2 className="w-4 h-4" />
                    </Button>
                  )}
                  <Button className="flex-1 bg-accent hover:bg-accent-hover text-white" onClick={handleSave} disabled={!amount || !categoryId}>
                    Save Budget
                  </Button>
                </div>
              </div>
